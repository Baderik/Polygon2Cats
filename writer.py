...
problem = etree.SubElement(cats, "Problem",
                           {
                               ...
                               "outputFile": p.io.output if p.io.output_file else "*STDOUT",
                               "saveInputPrefix": "50B",
...
copy_tests()
manual_count = sum(map(lambda el: el.method == "manual", p.tests.tests))
if manual_count:
...
generators = []
groups = {}
for t_i in range(len(p.tests.tests)):
    if p.tests.tests[t_i].method == "manual":
        continue
    test = etree.SubElement(problem, "Test", {"rank": str(t_i + 1)})
    g, *params = p.tests.tests[t_i].arguments.split(" ")
    generators.append(g)
    etree.SubElement(test, "In", {"param": " ".join(params), "use": g})
if len(p.tests.tests) == 1:
    test = etree.SubElement(problem, "Test", {"rank": str(1)})
else:
    test = etree.SubElement(problem, "Test", {"rank": f"1-{len(p.tests.tests)}"})
    etree.SubElement(test, "Out", {"use": sol})
add_generators(problem, generators)

cats.append(etree.Comment("This packet auto-generated by Baderik v0.2"))
